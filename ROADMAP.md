...
| Selective undo | `/undo` reverts only agent hunks and keeps later user edits (preview + redo). | Shipped (1.6.0: per-hunk 3-way merge in `safety.py`, `/undo preview`, `/undo --force`, `/undo [n] [path]`, `/redo`) |
